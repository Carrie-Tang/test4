...
class WSGIServer(object):
    ...
    def handle_one_request(self):
        ...
        content = env['PATH_INFO'][1:]
                   
        if env['PATH_INFO'][1:].endswith(".html"):
            if os.path.exists(content):
                response_headers = [('Content-Type', 'text/html')]
                self.start_response('200 OK', response_headers) 
                f = open(content,"r")             # 返回一个文件对象  
                line = f.readline()         # 调用文件的 readline()方法
                message = line
                while line:  
                    # print(line, end = '') # 在 Python 3中使用  
                    line = f.readline() 
                    message = message + line             # 后面跟 ',' 将忽略换行符 
                self.finish_response(message)
            else:
    ...
    def parse_request(self, text):
        ...
        # Break down the request line into cmponents
